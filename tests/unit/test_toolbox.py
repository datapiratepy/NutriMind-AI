...
class _FakeRAG:
    ...
    def __init__(self):
        self.provider = type("P", (), {"name": "lexical"})()
        self.retrieve_calls = 0

    def retrieve(self, query, *, top_k=None, document_ids=None):  # noqa: ARG002
...
def test_factory_is_not_called_when_no_retrieval_happens():
    ...
    calls = []
    toolbox = Toolbox(lambda: calls.append(1), user_id=1)
...
def test_factory_is_called_once_and_memoised():
    ...
    toolbox = Toolbox(factory, user_id=1)
...
def test_provider_reports_not_used_before_any_retrieval():
    ...
    assert Toolbox(lambda: _FakeRAG(), user_id=1).embedding_provider_name() == "not_used"


def test_provider_reports_real_name_after_retrieval():
    toolbox = Toolbox(lambda: _FakeRAG(), user_id=1)
    toolbox.retrieve_knowledge("protein")
    assert toolbox.embedding_provider_name() == "lexical"
...
def test_provider_reports_unavailable_after_failure():
    ...
    toolbox = Toolbox(boom, user_id=1)
    toolbox.retrieve_knowledge("protein")
...
def test_retrieval_failure_degrades_instead_of_raising(failure):
    ...
    result = Toolbox(boom, user_id=1).retrieve_knowledge("protein in paneer")
