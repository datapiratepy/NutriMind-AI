...
def get_engine():
    # Flask-Migrate's template tries the pre-3.0 `db.get_engine()` first and
    # falls back to `db.engine`. This project pins Flask-SQLAlchemy 3.x, where
    # `get_engine()` is deprecated and warns on every call, so the fallback is
    # made the only path.
